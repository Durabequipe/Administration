...
<x-tall-interactive::forms.form-container :controlsDesign="$controlsDesign" :maxWidth="$maxWidth"
        ...
        @if($showControls)
            <x-tall-interactive::forms.controls.base>
                <x-dynamic-component :component="'tall-interactive::forms.controls.' . $controlsDesign">
                    ...
                    <button wire:click="{{ $formClass ? 'submit': 'submitSlot' }}" type="submit"
                            class="tall-interactive-submit-button w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-primary-600 text-base font-medium text-white hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 sm:ml-3 sm:w-auto sm:text-sm">
                        {!! $submitWith !!}
                    </button>
