        ...
        <div class="absolute inset-0 overflow-hidden">
            ...
            <div class="absolute inset-0" aria-hidden="true">
                <div class="fixed inset-y-0 pl-8 sm:pl-16 max-w-full right-0 flex">
                    ...
                    >
                        <div
                                class="tall-interactive-slide-over-body h-full divide-y divide-gray-200 flex flex-col bg-white shadow-xl">
                                            ...
                                            <div class="flex items-center justify-between">
                                                ...
                                                @if($dismissable)
                                                    <div class="ml-3 h-7 flex items-center">
                                                        <button type="button"
                                                                wire:click="$emit('slideOver:close', '{{ $actionableId }}')"
                                                                class="hover:bg-primary-800 rounded-md text-primary-200 hover:text-white focus:outline-none focus:ring-2 focus:ring-white">
                                ...
                                @else
                                    @if($dismissable)
                                        <div class="absolute top-2 right-6 ml-3 h-7 flex items-center">
                                            <button type="button"
                                                    wire:click="$emit('slideOver:close', '{{ $actionableId }}')"
                                                    class="hover:bg-gray-100 rounded-md text-gray-400 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-primary-500">
                                                <span class="sr-only">{{ $dismissableWith }}</span>
                                                <!-- Heroicon name: outline/x -->
                                    ...
                                    <div class="my-8 px-4 divide-y divide-gray-200 sm:px-6">
                                        ...
                                        @if($formClass)
                                            <form wire:submit.prevent="submit">
                                                ...
                                                <button type="submit" class="invisible max-h-0 max-w-0 absolute">
                                                    Submit
                                                </button>
                                            </form>
                                        @elseif($livewire)
                            ...
                            @if($showControls)
                                <div class="flex-shrink-0 px-4 py-4 flex justify-end gap-2 flex-wrap"
                                     id="tall-interactive-slide-over-controls">
                                    @if($dismissable)
                                        <button wire:click="$emit('modal:close', '{{ $actionableId }}')" type="submit"
                                                class="tall-interactive-close-button w-full md:w-auto bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2
